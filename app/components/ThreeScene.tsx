  ...
  (
    ...
    useEffect(() => {
      ...
      const initTimer = setTimeout(() => {
        ...
        const colorPalette = NEBULA_COLOR_PALETTE.map(
          (c) => new THREE.Color(c)
        );
        ...
        const handleMouseMove = (event: MouseEvent) => {
          ...
          if (!isHovering) {
            ...
            if (
              nebulaIntersects.length > 0 &&
              nebulaIntersects[0].index !== undefined &&
              nebulaIntersects[0].distanceToRay !== undefined &&
              nebulaIntersects[0].distanceToRay < CLICK_RADIUS
            ) {
              isHovering = true;
            }
          }
          ...
          if (nebulaRef.current) {
            ...
            if (!isShapePaused) {
              ...
              if (
                ...
                console.log(
                  `[ThreeScene] shapeTimer: ${shapeTimerRef.current.toFixed(
                    2
                  )}s`
                );
              }
            ...
            if (
              ...
              settledParticlesRef.current.forEach((p, i) => {
                if (p.mesh) {
                  ...
                  settledParticlesTargetPositionsRef.current.set(
                    p.id,
                    targetPos
                  );
                  if (orbit) {
                ...
                cameraAnimationRef.current = {
                  ...
                  onComplete: () => {
                    ...
                    setTimeout(() => {
                      ...
                      shapeTransitionCameraStateRef.current.phase =
                        "zooming-in";
                      const originalPos =
                        shapeTransitionCameraStateRef.current
                          .originalCameraPos ||
            ...
            if (shapeTransitionTargetRef.current !== shapeModeRef.current) {
                ...
                if (shapeModeRef.current === "atomic") {
                  if (targetAtomicOrbitsRef.current) {
                    ...
                    atomicOrbitsRef.current =
                      generateAtomicOrbits(particleCount);
                  }
              ...
              const nebulaPositions = nebulaRef.current.geometry.attributes
                .position.array as Float32Array;
              const wavePositions = generateWaveShape(
                particleCount,
                elapsedTime
              );
            ...
            if (fadeValue > 0.01) {
              ...
              if (highlightId) {
                ...
                if (!targetPos) {
                  ...
                  if (
                    ...
                    const nebulaPositions = nebulaRef.current.geometry
              ...
              if (targetPos) {
                ...
                const highlightScale =
                  (6 + Math.sin(pulseTime) * 2) * smoothFade;
          ...
          for (let i = projectilesRef.current.length - 1; i >= 0; i--) {
            ...
            switch (p.phase) {
              ...
              case "settling": {
                ...
                const blink =
                  0.5 + blinkWave * 0.5 * params.settleBlinkAmplitude;
                (p.sprite.material as THREE.SpriteMaterial).opacity =
                  0.3 + blink * 0.7;
                const glowScale =
                  params.particleGlow *
                ...
                if (p.phaseProgress >= params.settleBlinkDuration) {
                  ...
                  console.log(
                    `[Settle Complete] Particle ${p.id} final state:`,
                    {
                      ...
                      finalWorldPos: p.mesh.getWorldPosition(
                        new THREE.Vector3()
                      ),
